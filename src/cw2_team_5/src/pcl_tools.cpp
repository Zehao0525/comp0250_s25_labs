...
void cw2::detect_objects(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr& in_cloud_ptr, DetectedObject detected_object, pcl::PointCloud<pcl::PointXYZRGBA>::Ptr& obj_cloud_ptr) {
    ...
    cec.setConditionFunction(&cw2::enforce_color_similarity);
  ...
  void cw2::detect_objects(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr& in_cloud_ptr, std::vector<DetectedObject>& detected_objects) {
    // 清空检测结果列表
  ...
  // Condition function 
bool cw2::enforce_color_similarity(const pcl::PointXYZRGBA& a, const pcl::PointXYZRGBA& b, float /*squared_dist*/) {
  ...
  void cw2::merge_clouds(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr new_cloud) {
